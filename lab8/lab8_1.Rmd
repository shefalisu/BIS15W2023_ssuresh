...
```{r}
#the column names except for patients are a problem because there is no name.

heartrate %>% 
  pivot_longer(-patient, #patients will not pivot or change
               names_to = "drug", #make a new column name 
               values_to = "heartrate") #value are moved to
```
...
#multiple pieces of data is put in the same variable
length_data %>% 
  transform(length = str_split(length, ";")) %>% #splits the data 
  unnest(length)
```
