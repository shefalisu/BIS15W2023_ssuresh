...
```{r}
ggplot(homerange, mapping = aes(x= log10.mass, y =log10.hra )) +geom_point()+geom_smooth(method = lm, se=T) #making line of best fit
#lm = linear model 
```
...
```{r}
ggplot(homerange, mapping = aes(x= log10.hra, y =log10.preymass)) +geom_point(na.rm = T)+geom_smooth(method = lm, se=F)
```
