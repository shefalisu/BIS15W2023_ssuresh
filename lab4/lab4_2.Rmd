...
```{r}
new_mammals_new <- select(mammals_new, family, genus, species, mass, gestation)
new_mammals_new
```

5. Use the output from #4 to focus on the family Bovidae with a mass greater than 450000.
```{r}
filter(new_mammals_new, family == "Bovidae" & mass > 450000)
```
...
```{r}
primate_genera <- filter(mammals_new, genus %int% c("Lophocebus", "Erythrocebus", "Macaca"))
```
