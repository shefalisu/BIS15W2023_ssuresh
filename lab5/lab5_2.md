...
## #   ⁴​skin_color, ⁵​alignment
```
We don't have 16 rows in question 10 because the functions evaluates if the data is a height over 300 OR a weight greater than or equal to 450. Since it is evaluating if the data meets one of those criteria, not all of the possible data values will be shown. 
...
```r
superhero_info %>% 
  mutate(ratio = height/weight, ratio) %>% 
  arrange(desc(ratio))
```
...
## # … with 724 more rows, 1 more variable: ratio <dbl>, and abbreviated variable
## #   names ¹​eye_color, ²​hair_color, ³​publisher, ⁴​skin_color, ⁵​alignment
```
