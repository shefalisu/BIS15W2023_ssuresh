...
#Changing many variables to factors (shift+command+m makes percent thing)
#look at the mammals data and if it is a character change it to a factor 
```{r}
...
#Connects the select and filter function (command+shift+m = pipe)
```{r}
fish %>%
...
```{r}
fish %>% 
  mutate(length_half = length/2, length_half) %>% 
